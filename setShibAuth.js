...
read({'prompt': 'Password: ', 'silent': true}, function(err, password) {
    ...
    UnityAPI.getIDPCSVData(argv.file, function(err, records) {
        if (err) {
            process.exit(1);
        }
...
var updateTenants = function(restCtx, tenants, records, callback) {
    ...
    // set gogole auth
    setIDPAuth(restCtx, tenants[record.alias], record, function(err, tenant) {
        if (err) {
...
var setIDPAuth = function(restCtx, tenant, record, callback) {
...
//    update['oae-authentication/google/enabled'] = true;
    ...
    // set to turn shib auth on
    update['oae-authentication/shibboleth/enabled'] = true;
